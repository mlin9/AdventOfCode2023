...
public class Day1 {
  ...
  public static int sumPartTwo(String[] document) {
    ...
    for (String line : document) {
      ...
      int first = parseString(matches.get(0)) * 10;
      int last = parseString(matches.get(matches.size() - 1));
      sum += first + last;
    }
    return sum;
  }
  ...
  public static ArrayList<String> regexFirstLast(String input, String pattern) {
    ...
    return new ArrayList<String>(Arrays.asList(matches.get(0), matches.get(matches.size() - 1)));
  }
